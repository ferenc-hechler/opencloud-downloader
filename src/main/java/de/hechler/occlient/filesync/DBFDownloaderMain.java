...
public class DBFDownloaderMain {
	...
	public static void main(String[] args) {
		...
		if (args == null || args.length == 0) {
			args = new String[] { "opencloud-dbf-downloader-syncs.yaml" };
		}
		...
		try {
			config = new OpenCloudConfig("opencloud-dbf-downloader.properties");
		...
		for (SyncEntry sync : sConf.sync) {
			...
			if (sync.transform != null && sync.transform.type == SyncTransformType.decrypt) {
				if (sync.transform.passphrase == null || sync.transform.passphrase.isEmpty()) {
					decryptPassphrase = config.getDefaultPassphrase();
				}
				else {
					decryptPassphrase = sync.transform.passphrase;
				}
				if (decryptPassphrase == null || decryptPassphrase.isEmpty()) {
					System.err.println("Decryption enabled for mapping " + remoteFolder + " -> " + localFolder + " but no passphrase provided in mapping or config");
					System.exit(4);
				}
			}
			System.out.println("Processing mapping: remote='" + remoteFolder + "' -> local='" + localFolder + "'"+ (decryptPassphrase != null ? " (decryption enabled)" : ""));
			if (localFolder.isEmpty() || remoteFolder.isEmpty()) {
				System.err.println("invalid mapping " + remoteFolder + " -> " + localFolder);
				System.exit(6);
			}
			folderSync.syncLocalFolder(localFolder, remoteFolder, sync.getIgnoreMatchers(), decryptPassphrase);
		}
	}
